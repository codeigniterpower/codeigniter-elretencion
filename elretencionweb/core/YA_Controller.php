...
//		$this->load->library('encrypt');
    ...
    public function checku()
    {
        ...
        {
			// IMPORTANTE si se va a algo que llame a check o login, volvera aqui
			if( $this->session->userdata('userlogin') == 1 )
				redirect('indexlogin/entrarlogin?sinpermiso='.$redirurl,'location');
			else
    ...
    public function genmenu($modulename = NULL)
    {
        ...
        if(($modulename == NULL OR $modulename == FALSE) AND $currentinx !== '')
        {
            ...
            $menumainstring .= ' '.anchor('/',ucfirst(APPCURREN),'class="active" ');
