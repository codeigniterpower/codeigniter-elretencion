...
class sys
{
	...
	/** rellena hasta 10 digitos ceros a la izquierda o los corta si esta demas */
	function completarceros($codigoocadena)
	{
		$codigoocadena = $this->completar_codigo($codigoocadena);
	...
	public function procesar_archivo_pedido_csv($campoinput, $modulo = '', $nombre = '')
	{
		...
		if(is_array($csv_array))
		{
			...
			{
				if(array_key_exists('columna',$arreglolinea))
						$columna .= $arreglolinea['list_codigos'].PHP_EOL;
				
			}
		}
		$retornar = array();
		$retornar['stringarchivo'] = $csv_array;
		return $retornar;
	}
}
